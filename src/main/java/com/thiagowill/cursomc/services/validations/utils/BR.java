package com.thiagowill.cursomc.services.validations.utils;

//fonte https://github.com/heitorbarbosaramos/validaDocumentos/blob/master/src/validaDocumentos/ValidaDocumentos.java
public class BR {
	...
	private static String replace(String numDoc) {
		return numDoc.replace(".", "").replace("-", "").replace("/", "").replace(" ", "");
	}
	...
	private static boolean calculador(String numDoc, Integer[] multiplicadores, Integer quantDigVer) {
		...
		for (int y = h; y >= 0; y--) {
			...
			if (!dig.equals(Integer.parseInt(numDoc.substring(a, b)))) {
				return false;
			}
		}
		return true;
	...
	public static boolean validaCNPJ(String numDoc) {
		Boolean result = false;
		if (numDoc.matches("\\d{2}\\.\\d{3}\\.\\d{3}/\\d{4}-\\d{2}") || numDoc.matches("\\d{14}")
				|| numDoc.matches("\\d{8}/\\d{4}-\\d{2}") || numDoc.matches("\\d{12}-\\d{2}")) {
	...
	public static boolean validaCPF(String numDoc) {
		Boolean result = false;
		if (numDoc.matches("\\d{3}\\.\\d{3}\\.?\\d{3}-\\d{2}") || numDoc.matches("\\d{11}")
				|| numDoc.matches("\\d{9}-\\d{2}")) {
